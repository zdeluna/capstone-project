...
exports.update = [
  ...
  check("date_of_birth")
    .optional()
    /* Check date to make sure it is in the YYYY_MM_DD format
    ...
    .custom((dateValue, { req }) => {
      // Check to make sure it is in YYYY_MM_DD format
      if (!dateValue.match(/^\d{4}-\d{2}-\d{2}$/)) return false;
      ...
      // Change the date in milliseconds and change it to the YYYY_MM_DD format and compare it to the value that was passed in.
      return date.toISOString().slice(0, 10) === dateValue;
    })
    .withMessage("DATE_OF_BIRTH_MUST_BE_IN_YYYY_MM_DD"),
